        ...
        NavigationView {
            ZStack {
                ...
                List {
                        ...
                        NavigationLink(destination: PipelineDebugView()) {
                            HStack {
                                Image(systemName: "chart.bar.doc.horizontal")
                                    .foregroundColor(WealthyRabbitTheme.terracotta)
                                Text("Pipeline Debug (App)")
                                    .font(WealthyRabbitTheme.bodyFont)
                            }
                        }
                        
                        Button(action: {
                            PipelineDebugServer.shared.start()
                        }) {
                            HStack {
                                Image(systemName: "globe")
                                    .foregroundColor(WealthyRabbitTheme.terracotta)
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("Pipeline Debug (Web)")
                                        .font(WealthyRabbitTheme.bodyFont)
                    ...
                    // Rabbit Mode Section
                    Section(header: Text("Rabbit Mode")) {
                        NavigationLink(destination: RabbitModeSettingsView(viewModel: viewModel)) {
                            HStack {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    Text("Rabbit Mode")
                                        .font(WealthyRabbitTheme.bodyFont)
                                    Text(viewModel.userSettings.rabbitMode.rawValue)
                                        .font(WealthyRabbitTheme.captionFont)
                                        .foregroundColor(.secondary)
                                }
                            }
                    ...
                    Section(header: Text("Push Notifications")) {
                        HStack {
                            Image(systemName: pushManager.notificationPermissionGranted ? "bell.fill" : "bell.slash")
                                .foregroundColor(pushManager.notificationPermissionGranted ? WealthyRabbitTheme.primaryColor : .secondary)
...
// MARK: - Rabbit Mode Settings View
struct RabbitModeSettingsView: View {
    @ObservedObject var viewModel: RabbitViewModel
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(spacing: WealthyRabbitTheme.airySpacing) {
                    ...
                    VStack(spacing: 8) {
                        ...
                        Text("Choose how the Rabbit Feed adapts to your investing experience")
                            .font(WealthyRabbitTheme.bodyFont)
                            .foregroundColor(.secondary)
                            .multilineTextAlignment(.center)
                            .padding(.horizontal, WealthyRabbitTheme.airySpacing)
                    ...
                    // Mode Options
                    VStack(spacing: 16) {
                        ForEach(RabbitMode.allCases, id: \.self) { mode in
                            RabbitModeOptionCard(
                                mode: mode,
                                isSelected: userSettings.rabbitMode == mode,
                                action: {
                                    userSettings.rabbitMode = mode
                                }
                            )
                        }
                    }
        ...
        .navigationTitle("Rabbit Mode")
        .navigationBarTitleDisplayMode(.inline)
    }
}

struct RabbitModeOptionCard: View {
    let mode: RabbitMode
    let isSelected: Bool
    let action: () -> Void
        ...
        Button(action: action) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    case .beginner:
                        VStack(alignment: .leading, spacing: 4) {
                            Text("• More explanations in cards and deep dives")
                            Text("• Extra context sections")
                            Text("• More knowledge checks")
                            Text("• General market events and discovery cards")
                        }
                    case .smart:
                        VStack(alignment: .leading, spacing: 4) {
                            Text("• Holdings-related cards + major macro events")
                            Text("• Normal frequency for knowledge checks")
                            Text("• Context only when needed")
                        }
                    case .focus:
                        VStack(alignment: .leading, spacing: 4) {
                            Text("• Only cards directly tied to holdings")
                            Text("• Hide global events unless they impact holdings")
                            Text("• Reduced knowledge checks")
                            Text("• Extremely clean feed")
                        }
                    }
                }
                .font(WealthyRabbitTheme.captionFont)
            ...
            .padding(WealthyRabbitTheme.normalSpacing)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(isSelected ? WealthyRabbitTheme.mossGreen.opacity(0.15) : Color.white.opacity(0.8))
            .cornerRadius(16)
            .overlay(
